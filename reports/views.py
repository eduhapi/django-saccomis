    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if start_date and end_date:
                ...
                transactions = transactions.filter(updated_date__range=(start_date, end_date))
...
def loan_balances(request):
    ...
    for loan in issued_loans:
        ...
        total_months = (today.year - repayments_start_date.year) * 12 + today.month - repayments_start_date.month
        interest_rate = Decimal('0.01')

        current_loan_balance = loan.loan_amount
        for i in range(total_months):
            current_loan_balance += current_loan_balance * interest_rate

        installments_paid = (today - repayments_start_date).days // 28
        missed_installments = installments_paid - total_months
        arrears_amount = max(0, missed_installments * loan.installment_amount)
        arrears_period = max(0, missed_installments)
        ...
        # Update the IssuedLoan with the new loan balance
        loan.loan_balance = current_loan_balance
        loan.save()
        ...
        loan_balances.append({
            ...
            'remaining_repayment_period': max(0, loan.repayment_period - installments_paid),
